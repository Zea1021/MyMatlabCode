...
InstanceName = AllInstance{2, CaseNo};
num = 20;
file = {'IMOGA/', 'NSGA-II/', 'PSOGA/', 'MDFFO/', 'MODE/', 'mMOEAD/', 'EDPSO/'};
algorithm = {'IMOGA', 'NSGA-II', 'PSOGA', 'MDFFO', 'MODE', 'mMOEA/D', 'EDPSO'};
colorAndShape = {'o-b', '*-r', 'd-g', 'x-k' 's-c', '-p', 'h-m'}; 
...
figure(1)
for k = 1 : algoNum
    load([file{k} prefix data suffix num2str(bestIdx(k))]);
    [~, Sort] = sort(elite_fit, 'ascend');
    elite_fit = elite_fit(Sort, :);
    plot(elite_fit(:, 1), elite_fit(:, 2),colorAndShape{k})
    hold on;
end
legend(algorithm);
title(InstanceName);
