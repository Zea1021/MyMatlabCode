function [p1_chrom, m1_chrom, p2_chrom, m2_chrom, fit] = initMethod3(Pi)
...
for i = 1 : N
    ...
    if rand <= Pi
        m = 1;
        for j = 2 : M1
            if (CM1(j) * Time1(job, j) < CM1(m) * Time1(job, m))
                m = j;
            end
        end
    else
...
for i = 1 : SH
    ...
    if rand <= Pi
        m = 1;
        for j = 2 : M2
            if (CM2(j) * Time2(job, compo, k, j) < CM2(m) * Time2(job, compo, k, m))
                m = j;
            end
        end
    else
